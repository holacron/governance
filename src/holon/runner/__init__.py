"""The minimal autonomous runner — Holon's first real product (ROADMAP §11).
...
from holon.config import REPO_ROOT, RuntimeConfig, load_runtime_config
from holon.gateway import CostCapExceeded, LLMGateway
from holon.store import RunnerStateRow, make_engine
...
class Runner:
    ...
    def run(self, tasks: list[Task], instance_id: str = "holon-bootstrap") -> None:
    ...
    def _execute_task(self, task: Task) -> TaskResult:
        ...
        resp = self.gateway.call_agent(
            role="orchestrator",
            prompt=(
                f"You are executing a development task in the Holon repo.\n"
                f"Task: {task}\n\n"
